...
class HashlibBenchmark extends SyncBenchmark {
  ...
  @override
  dynamic run() {
    return hashlib.HMAC(hashlib.md5).by(key).convert(input).bytes;
  }
}
...
class CryptoBenchmark extends SyncBenchmark {
  ...
  @override
  dynamic run() {
    return crypto.Hmac(crypto.md5, key).convert(input).bytes;
  }
}
...
class HashBenchmark extends SyncBenchmark {
  ...
  @override
  dynamic run() {
    return hash.Hmac(hash.MD5(), key).update(input).digest();
  }
}
