...
final tests = {
  ...
  List.filled(513, "a").join():
      "c3f1d47c5dad5c3b8cc1242da14220af7f9036acf98369be3cc102b069476a9b3e50b9a131756396c8c267dce06a35f0",
  List.filled(511, "a").join():
      "db100a1eed3842c61d73064b62543cd4531dafa8bfecf6f27dcfdebbaf60ea14563ea1b486e4f6b9a14fcb0dac05c5f2",
...
void main() {
  group('SHA384 test', () {
    test('with empty string', () {
      expect(hashlib.sha384sum("").hex(), tests[""]);
    });

    test('with single letter', () {
      expect(hashlib.sha384sum("a").hex(), tests["a"]);
    });

    test('with few letters', () {
      expect(hashlib.sha384sum("abc").hex(), tests["abc"]);
    });

    test('with string of length 511', () {
      var key = tests.keys.firstWhere((x) => x.length == 511);
      var value = tests[key]!;
      expect(hashlib.sha384sum(key).hex(), value);
    });
    ...
    test('with stream', () async {
      for (final entry in tests.entries) {
        final stream = Stream.fromIterable(
            ...
            .map(toBytes);
        final result = await hashlib.sha384stream(stream);
    ...
    test('to compare against known implementations', () {
      for (int i = 0; i < 1000; ++i) {
        ...
        expect(
          toHex(hashlib.sha384.convert(data).bytes),
