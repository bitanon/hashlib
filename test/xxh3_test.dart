...
const seed = (0x9e3779b1 << 32) | 0x85ebca8d;
final cases = fromHex(
...
String sum([Iterable<int>? data]) {
  return xxh3.convert(data?.toList() ?? []).hex(true);
}

String seedSum(int seed, [Iterable<int>? data]) {
  return XXH3.withSeed(seed).convert(data?.toList() ?? []).hex(true);
}

String secretSum([Iterable<int>? data]) {
  return XXH3.withSecret(secret).convert(data?.toList() ?? []).hex(true);
}

void main() {
  group('XXH3 test', () {
    test("with seed = 0, length = 0", () {
      expect(sum(), "776EDDFB6BFD9195");
    });
    test("with seed = $seed, length = 0", () {
      expect(seedSum(seed), "6AFCE90814C488CB");
    });
    test("with seed = 0, length = 1", () {
      expect(sum(cases.take(1)), "B936EBAE24CB01C5");
    });
    test("with seed = $seed, length = 1", () {
      expect(seedSum(seed, cases.take(1)), "F541B1905037FC39");
    });
    test("with seed = 0, length = 6", () {
      expect(sum(cases.take(6)), "27B56A84CD2D7325");
    });
    test("with seed = $seed, length = 6", () {
      expect(seedSum(seed, cases.take(6)), "84589C116AB59AB9");
    });
    test("with seed = 0, length = 12", () {
      expect(sum(cases.take(12)), "A713DAF0DFBB77E7");
    });
    test("with seed = $seed, length = 12", () {
      expect(seedSum(seed, cases.take(12)), "E7303E1B2336DE0E");
    });
    test("with seed = 0, length = 24", () {
      expect(sum(cases.take(24)), "A3FE70BF9D3510EB");
    });
    test("with seed = $seed, length = 24", () {
      expect(seedSum(seed, cases.take(24)), "850E80FC35BDD690");
    });
    test("with seed = 0, length = 48", () {
      expect(sum(cases.take(48)), "397DA259ECBA1F11");
    });
    test("with seed = $seed, length = 48", () {
      expect(seedSum(seed, cases.take(48)), "ADC2CBAA44ACC616");
    });
    test("with seed = 0, length = 80", () {
      expect(sum(cases.take(80)), "BCDEFBBB2C47C90A");
    });
    test("with seed = $seed, length = 80", () {
      expect(seedSum(seed, cases.take(80)), "C6DD0CB699532E73");
    });
    test("with seed = 0, length = 195", () {
      expect(sum(cases.take(195)), "CD94217EE362EC3A");
    });
    test("with seed = $seed, length = 195", () {
      expect(seedSum(seed, cases.take(195)), "BA68003D370CB3D9");
    });
    test("with seed = 0, length = 403", () {
      expect(sum(cases.take(403)), "1B2AFF3B46C74648");
    });
    test("with seed = $seed, length = 403", () {
      expect(seedSum(seed, cases.take(403)), "B654F6FFF42AD787");
    });
    test("with seed = 0, length = 512", () {
      expect(sum(cases.take(512)), "43E368661808A9E8");
    });
    test("with seed = $seed, length = 512", () {
      expect(seedSum(seed, cases.take(512)), "3A865148E584E5B9");
    });
    test("with seed = 0, length = 2048", () {
      expect(sum(cases.take(2048)), "C7169244BBDA8BD4");
    });
    test("with seed = $seed, length = 2048", () {
      expect(seedSum(seed, cases.take(2048)), "74BF9A802BBDFBAE");
    });
    test("with seed = 0, length = 2240", () {
      expect(sum(cases.take(2240)), "30FEB637E114C0C7");
    });
    test("with seed = $seed, length = 2240", () {
      expect(seedSum(seed, cases.take(2240)), "EEF78A36185EB61F");
    });
    test("with seed = 0, length = 2243", () {
      expect(sum(cases.take(2243)), "62C631454648A193");
    });
    test("with seed = $seed, length = 2243", () {
      expect(seedSum(seed, cases.take(2243)), "6CF80A4BADEA4428");
    });
    ...
    test("with secret, length = 0", () {
      expect(secretSum(), "6775FD10343C92C3");
    });

    test("with secret, length = 1", () {
      expect(secretSum(cases.take(1)), "C3382C326E24E3CD");
    });

    test("with secret, length = 6", () {
      expect(secretSum(cases.take(6)), "82C90AB0519369AD");
    });

    test("with secret, length = 12", () {
      expect(secretSum(cases.take(12)), "14631E773B78EC57");
    });

    test("with secret, length = 24", () {
      expect(secretSum(cases.take(24)), "CDD5542E4A9D9FE8");
    });

    test("with secret, length = 48", () {
      expect(secretSum(cases.take(48)), "33ABD54D094B2534");
    });

    test("with secret, length = 80", () {
      expect(secretSum(cases.take(80)), "E687BA1684965297");
    });

    test("with secret, length = 195", () {
      expect(secretSum(cases.take(195)), "A057273F5EECFB20");
    });

    test("with secret, length = 403", () {
      expect(secretSum(cases.take(403)), "F9C0BA5BA3AF70B8");
    });

    test("with secret, length = 512", () {
      expect(secretSum(cases.take(512)), "7896E65DCFA09071");
    });

    test("with secret, length = 2048", () {
      expect(secretSum(cases.take(2048)), "D6545DB87ECFD98C");
    });

    test("with secret, length = 2243", () {
      expect(secretSum(cases.take(2243)), "887810081C32460A");
    });
  }, tags: 'skip-js');
}
