// Copyright (c) 2021, Sudipto Chandra
// All rights reserved. Check LICENSE file for details.

import 'package:hashlib/hashlib.dart';
import 'package:hashlib/src/core/utils.dart';
import 'package:test/test.dart';

const seed = (0x9e3779b1 << 32) | 0x85ebca8d;
final cases = fromHex(
    '');

String sum(int seed, [Iterable<int>? data]) {
  return XXH3.withSeed(seed).convert(data?.toList() ?? []).hex(true);
}

void main() {
  group('XXH3 test', () {
    test("with seed = 0, length = 0", () {
      expect(sum(0), "776EDDFB6BFD9195");
    });
    test("with seed = $seed, length = 0", () {
      expect(sum(seed), "6AFCE90814C488CB");
    });
    test("with seed = 0, length = 1", () {
      expect(sum(0, cases.take(1)), "B936EBAE24CB01C5");
    });
    test("with seed = $seed, length = 1", () {
      expect(sum(seed, cases.take(1)), "F541B1905037FC39");
    });
    test("with seed = 0, length = 6", () {
      expect(sum(0, cases.take(6)), "27B56A84CD2D7325");
    });
    test("with seed = $seed, length = 6", () {
      expect(sum(seed, cases.take(6)), "84589C116AB59AB9");
    });
    test("with seed = 0, length = 12", () {
      expect(sum(0, cases.take(12)), "A713DAF0DFBB77E7");
    });
    test("with seed = $seed, length = 12", () {
      expect(sum(seed, cases.take(12)), "E7303E1B2336DE0E");
    });
    test("with seed = 0, length = 24", () {
      expect(sum(0, cases.take(24)), "A3FE70BF9D3510EB");
    });
    test("with seed = $seed, length = 24", () {
      expect(sum(seed, cases.take(24)), "850E80FC35BDD690");
    });
    test("with seed = 0, length = 48", () {
      expect(sum(0, cases.take(48)), "397DA259ECBA1F11");
    });
    test("with seed = $seed, length = 48", () {
      expect(sum(seed, cases.take(48)), "ADC2CBAA44ACC616");
    });
    test("with seed = 0, length = 80", () {
      expect(sum(0, cases.take(80)), "BCDEFBBB2C47C90A");
    });
    test("with seed = $seed, length = 80", () {
      expect(sum(seed, cases.take(80)), "C6DD0CB699532E73");
    });
    test("with seed = 0, length = 195", () {
      expect(sum(0, cases.take(195)), "CD94217EE362EC3A");
    });
    test("with seed = $seed, length = 195", () {
      expect(sum(seed, cases.take(195)), "BA68003D370CB3D9");
    });
    test("with seed = 0, length = 403", () {
      expect(sum(0, cases.take(403)), "1B2AFF3B46C74648");
    });
    test("with seed = $seed, length = 403", () {
      expect(sum(seed, cases.take(403)), "B654F6FFF42AD787");
    });
    test("with seed = 0, length = 512", () {
      expect(sum(0, cases.take(512)), "43E368661808A9E8");
    });
    test("with seed = $seed, length = 512", () {
      expect(sum(seed, cases.take(512)), "3A865148E584E5B9");
    });
    test("with seed = 0, length = 2048", () {
      expect(sum(0, cases.take(2048)), "C7169244BBDA8BD4");
    });
    test("with seed = $seed, length = 2048", () {
      expect(sum(seed, cases.take(2048)), "74BF9A802BBDFBAE");
    });
    test("with seed = 0, length = 2240", () {
      expect(sum(0, cases.take(2240)), "30FEB637E114C0C7");
    });
    test("with seed = $seed, length = 2240", () {
      expect(sum(seed, cases.take(2240)), "EEF78A36185EB61F");
    });
    test("with seed = 0, length = 2243", () {
      expect(sum(0, cases.take(2243)), "62C631454648A193");
    });
    test("with seed = $seed, length = 2243", () {
      expect(sum(seed, cases.take(2243)), "6CF80A4BADEA4428");
    });
  }, tags: 'skip-js');
}
